document.write ('<link href="//fonts.googleapis.com/css?family=Ubuntu:400,700" rel="stylesheet" type="text/css">');
...
function getEnclosureUrl (item) { //11/23/23 by Andy Sylvester
	if (riverBrowserData.flEnclosureIcon) {
		var enclosurelink = "";
		if (item.enclosure != undefined) {
			var theEnclosure = item.enclosure [0];
			if ((theEnclosure != undefined) && (theEnclosure.url != undefined)) {
				...
				enclosurelink = "<img src=\"" + theEnclosure.url + "\">";
			}
		return (enclosurelink);
		}
	else {
...
function renderRiveritem (item, serialnum) { 
	...
	if (renderThroughOutlineBrowser (item)) {
		...
		var itembody = "<div class=\"divItemDescription\">" + body + getEnclosureUrl(item) + "</div>";
